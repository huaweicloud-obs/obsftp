...
import time
from .const import LONG, BASESTRING
from . import util
...
class ResponseWrapper(object):
    def __init__(self, conn, result, connHolder, contentLength=None):
        self.conn = conn
        self.result = result
    ...
    def __getattr__(self, name):
        if name == 'read' and self.result and self.contentLength is not None:
            def _read(*args, **kwargs):
                chunk = self.result.read(*args, **kwargs)
                if not chunk:
                    if self.contentLength != self.readedCount:
