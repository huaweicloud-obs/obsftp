...
import FTPOpCallback

class AbstrFileToObs(AbstractedFS):
    ...
    def open(self, filename, mode):
        ...
        if mode.startswith('r') or mode.startswith('R'):
            return self.obs_fs_callback.open_read(filename)
        else:
            return self.obs_fs_callback.open_write(filename)
    ...
    def mkdir(self, path):
        assert isinstance(path, unicode), path
        self.obs_fs_callback.mkdir(path)
        
    def infopath(self, path):
        assert isinstance(path, unicode), path
        self.obs_fs_callback.infopath(path)
    
    def listdir(self, path):
        assert isinstance(path, unicode), path
        return self.obs_fs_callback.listdir(path)
        
    def rmdir(self, path):
        assert isinstance(path, unicode), path
        self.obs_fs_callback.rmdir(path)
        
    def remove(self, path):
        assert isinstance(path, unicode), path
        self.obs_fs_callback.remove(path)
    
    def lexists(self, path):
        assert isinstance(path, unicode), path
        return self.obs_fs_callback.lexists(path)

    def rename(self, src, dst):
        assert isinstance(src, unicode), src
        assert isinstance(dst, unicode), dst
        self.obs_fs_callback.rename(src, dst)
    ...
    def getsize(self, path):
        assert isinstance(path, unicode), path
        return self.obs_fs_callback.getsize(path)
    
    def getmtime(self, path):
        assert isinstance(path, unicode), path
        return self.obs_fs_callback.getmtime(path)
    ...
    def isfile(self, path):
        assert isinstance(path, unicode), path
        return self.obs_fs_callback.isfile(path)
    
    def isdir(self, path):
        assert isinstance(path, unicode), path
        return self.obs_fs_callback.isdir(path)
